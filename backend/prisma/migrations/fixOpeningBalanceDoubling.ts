  ...
  for (const supplier of suppliers) {
    ...
    if (existingVoucher) {
      ...
      if (!dryRun && eqLedger) {
        ...
        let supplierLedger = await prisma.accountLedger.findUnique({ where: { supplierId: supplier.id } });
        if (!supplierLedger) {
          const ledgerCode = `SUPP-${supplier.supplierCode}`;
          supplierLedger = await prisma.accountLedger.create({
            data: {
              code: ledgerCode,
              ...
              create: [
                {
                  ...
                  debitAmount: new Prisma.Decimal(opBal),
                ...
                },
                {
                  ...
                  creditAmount: new Prisma.Decimal(opBal),
  ...
  for (const customer of customers) {
    ...
    if (existingVoucher) {
      ...
      if (!dryRun && eqLedger) {
        ...
        let customerLedger = await prisma.accountLedger.findUnique({ where: { customerId: customer.id } });
        if (!customerLedger) {
          const ledgerCode = `CUST-${customer.customerCode}`;
          customerLedger = await prisma.accountLedger.create({
            data: {
              code: ledgerCode,
