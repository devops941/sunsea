...
exports.backfillOpeningBalanceVouchers = backfillOpeningBalanceVouchers;
const prisma_1 = require("../../src/config/prisma");
const accounts_service_1 = require("../../src/modules/accounts/accounts.service");
const voucherPosting_service_1 = require("../../src/modules/accounts/voucherPosting.service");
/**
 ...
 */
async function backfillOpeningBalanceVouchers() {
    console.log("🚀 Starting Opening Balance Vouchers Backfill Migration...");
    // Ensure system ledgers (including EQ-001) exist
    await accounts_service_1.accountsService.ensureSystemLedgersExist();
    ...
    for (const supplier of suppliers) {
        ...
        if (opBal > 0) {
            ...
            if (!existingVoucher) {
                const opBalType = (supplier.openingBalanceType || "CREDIT").toUpperCase();
                await voucherPosting_service_1.voucherPostingService.postSupplierOpeningBalanceVoucher({ id: supplier.id, supplierCode: supplier.supplierCode, legalName: supplier.legalName }, opBal, opBalType);
                supplierBackfills++;
    ...
    for (const customer of customers) {
        ...
        if (opBal > 0) {
            ...
            if (!existingVoucher) {
                const opBalType = (customer.openingBalanceType || "DEBIT").toUpperCase();
                await voucherPosting_service_1.voucherPostingService.postCustomerOpeningBalanceVoucher({ id: customer.id, customerCode: customer.customerCode, firmName: customer.firmName }, opBal, opBalType);
                customerBackfills++;
