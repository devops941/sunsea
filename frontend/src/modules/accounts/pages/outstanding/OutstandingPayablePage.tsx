...
import { FaChevronDown, FaChevronRight, FaSync, FaFileInvoiceDollar, FaDownload } from "react-icons/fa";
...
const OutstandingPayablePage: React.FC = () => {
  ...
  const { data: reportList, refreshing, refresh } = useListCache<OutstandingReport>({
    cacheKey: `accounts:outstanding-payable:${asOnDate}`,
  ...
  const exportCSV = () => {
    if (!report) return;
    const rows: string[][] = [];
    rows.push(["Code", "Supplier", "Invoice No", "Invoice Date", "Due Date", "Bill Amount", "Paid", "Balance", "Age Days", "Bucket"]);
    ...
    a.href = url;
    a.download = `Outstanding_Payable_${asOnDate}.csv`;
    a.click();
    ...
    <div className="p-3 space-y-3 min-h-screen font-sans text-ink ">
      ...
      <div className="bg-card rounded-lg border border-line px-3 py-2 flex flex-wrap items-center gap-2">
        <h1 className="text-sm font-bold text-ink flex items-center gap-2 mr-2">
          <FaFileInvoiceDollar className="text-rose-500 text-sm" /> Outstanding Payable
          <span className="text-[10px] font-semibold px-1.5 py-0.5 bg-rose-500/10 text-rose-500 border border-rose-500/30 rounded uppercase tracking-wide">
            Bill-wise
          </span>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <input
            ...
            className="w-[140px] px-2 py-1 border border-line bg-card rounded text-xs text-ink focus:ring-1 focus:ring-rose-500/40 focus:border-rose-500 focus:outline-none"
          />
        </div>
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          <button onClick={exportCSV} disabled={!report || report.parties.length === 0}
            className="flex items-center gap-1 px-2 py-1 bg-rose-600 hover:bg-rose-700 text-white rounded text-xs font-semibold disabled:opacity-50">
      ...
      {report && (
        <div className="bg-card border border-line rounded-lg px-3 py-2 flex items-center justify-between">
          <span className="text-[11px] uppercase tracking-wide font-bold text-ink-subtle">
            Grand Total ({report.parties.length} suppliers · {report.parties.reduce((s, p) => s + p.bills.length, 0)} bills)
          </span>
            ...
            <tbody className="divide-y divide-line-soft">
              {!report || report.parties.length === 0 ? (
                <tr><td colSpan={9} className="p-8 text-center text-xs text-ink-subtle">
                  {refreshing ? null : "No outstanding payables. All suppliers are settled."}
                </td></tr>
              ) : (
...
export default OutstandingPayablePage;
