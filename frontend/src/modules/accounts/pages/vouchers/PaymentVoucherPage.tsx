...
const PaymentVoucherPage: React.FC = () => {
  ...
  // Columns are dynamic: 0=Date, 1=Vch/Bill No, [Mode if showPaidFrom],
  //                      then Account, [Amount if showAmount], [Narration if showNarration]
  const [colIdx, setColIdx] = useState<number>(0);
  const colCount = useMemo(() => {
    return 3 // Date + Vch + Account (always shown)
      + (applied.showPaidFrom ? 1 : 0)
      + (applied.showAmount ? 1 : 0)
      + (applied.showNarration ? 1 : 0);
  }, [applied.showPaidFrom, applied.showAmount, applied.showNarration]);
  // Clamp column when toggles change
  useEffect(() => {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      e.preventDefault();
      e.stopPropagation();
      // Filter panel open Esc → walk back one page in history.
            ...
            <div className="p-4 space-y-2.5 text-[13px]">
              ...
              <div className="pt-2 flex justify-center">
                <button
                  ...
                  className="px-6 py-1 bg-red-600 hover:bg-red-700 text-white rounded font-semibold text-[13px] transition cursor-pointer"
                >
                  OK (F2)
                </button>
              </div>
    ...
    <div data-escape-guarded className="p-3 space-y-2 w-full max-w-7xl">
      ...
      <div className="bg-card rounded-md border border-line px-3 py-1.5 flex flex-wrap items-center gap-2 shadow-sm">
        <h1 className="text-[13px] font-bold text-ink flex items-center gap-2 mr-2">
          <FaMoneyBillWave className="text-red-500 text-[13px]" /> Payment Register
          {refreshing && <FaSync className="animate-spin text-red-500 text-[13px]" />}
        </h1>
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          >
            <FaPlus className="text-[13px]" /> New Payment
          </button>
        </div>
      </div>
          ...
          <div className="p-8 text-center text-xs text-ink-subtle">
            {loading ? (
              <span className="inline-flex items-center gap-2">
                <FaSync className="animate-spin text-red-500 text-[13px]" />
            ...
            <table className="w-full text-left text-[13px] text-ink-muted border-collapse">
              ...
              <tbody>
                {vouchers.map((v, rowIdx) => {
                  ...
                  // Cell ring — active voucher + active column. Column indices
                  // depend on which optional columns are visible.
                  let cursor = 0;
                  const dateCol = cursor++;
                  const vchCol = cursor++;
                  const modeCol = applied.showPaidFrom ? cursor++ : -1;
                  const accCol = cursor++;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td onClick={clickFor(vchCol)} className={`px-2 py-1 border-r border-line-soft font-mono font-semibold text-red-500 text-center${ringFor(vchCol)}`}>
                ...
                <tfoot className="sticky bottom-0 z-10 bg-card-2 border-t-2 border-line">
                  <tr>
                    <td
                      ...
                      className="px-2 py-1.5 text-right text-[13px] font-bold text-ink uppercase tracking-wide border-r border-line"
                    >
