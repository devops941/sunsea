  ...
  return (
    <div className="w-full p-4 md:p-6 bg-slate-50 min-h-screen font-sans text-slate-800 space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200">
        {/* Header Title Bar */}
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 p-4 border-b border-slate-200 rounded-t-2xl">
          <div>
        ...
        <div className="px-6 py-3 bg-slate-50/50 flex items-center gap-2 overflow-x-auto border-b border-slate-100">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500 font-mono">Total Count: {total}</span>
        </div>

        {loading ? (
          <div className="p-8 text-center text-slate-500">Loading vouchers...</div>
        ) : vouchers.length === 0 ? (
          <div className="p-12 text-center text-slate-400">No vouchers matching your filter criteria.</div>
        ) : (
                ...
                {vouchers.map((v) => {
                  ...
                  return (
                    <tr key={v.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-4 py-3 font-medium text-slate-900">{debitLedgerName}</td>
                      <td className="px-4 py-3 text-slate-600">{creditLedgerName}</td>
                      {/* Formatted Amount */}
                      ...
                      <td className="px-4 py-3 text-slate-500 max-w-xs truncate" title={v.narration || ""}>
                        {v.narration || "-"}
                      </td>
                      {/* Actions Column */}
                      {typeFilter !== "PAYMENT" && (
        ...
        {totalPages > 1 && (
          <div className="p-4 border-t border-slate-200 bg-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-xs text-slate-500 font-medium">
              ...
              <span className="font-semibold text-slate-800">{total}</span> vouchers
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2 border border-slate-300 rounded-lg text-slate-600 hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition"
              >
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                <div>
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Invoice Number</span>
                  <span className="font-bold text-slate-800">{selectedVoucher.refDoc?.invoiceNo || "-"}</span>
                </div>
                <div>
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">GRN / PO Ref</span>
                  <span className="font-mono font-medium text-slate-800">{getGrnRef(selectedVoucher)}</span>
                </div>
                <div>
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Supplier / Customer</span>
                      ...
                      <tbody className="divide-y divide-slate-200 text-slate-600">
                        {selectedVoucher.refDoc.items.map((item, idx) => (
                          <tr key={idx} className="hover:bg-slate-50">
                            ...
                            <td className="px-3 py-2 text-right font-bold text-slate-800">{item.quantity}</td>
              ...
              <div>
                <h3 className="font-semibold text-slate-800 mb-2 flex items-center gap-2">
                  <FaInfoCircle className="text-slate-500" /> Journal Postings (Debit / Credit Ledgers)
                </h3>
                <div className="border border-slate-200 rounded-lg overflow-hidden">
                  <table className="w-full text-left text-xs">
                    <thead className="bg-slate-100 text-slate-700 font-semibold uppercase">
              ...
              {selectedVoucher.narration && (
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-1">Narration</span>
                  <p className="text-slate-700 italic">{selectedVoucher.narration}</p>
                </div>
              )}
            </div>
