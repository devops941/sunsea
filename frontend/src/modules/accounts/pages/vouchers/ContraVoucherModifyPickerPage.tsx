...
const ContraVoucherModifyPickerPage: React.FC = () => {
  ...
  const { data: cachedVouchers } = useListCache<Voucher>({
    cacheKey: `accounts:contra-vouchers:modify-picker`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (vNoRaw) {
        // Normalise "3" or "C-3" -> "CTR-3".
        let normalized = vNoRaw.toUpperCase();
        if (/^\d+$/.test(vNoRaw)) {
          normalized = `CTR-${vNoRaw}`;
        } else if (/^C-\d+$/.test(normalized)) {
          normalized = normalized.replace(/^C-/, "CTR-");
        }

        const cacheHit = cachedVouchers.find((v) => v.voucherNo === normalized);
        if (cacheHit) {
          navigate(`/accounts/contra-entry/edit/${cacheHit.id}`, {
        ...
        if (!found) {
          toast.error(`Voucher "${vNoRaw}" not found in Contra`);
          return;
        }
        navigate(`/accounts/contra-entry/edit/${found.id}`, {
          state: { voucher: found },
        });
        return;
      }

      // Open a voucher on the selected date, or fall back to the most
      // recent voucher AT OR BEFORE that date (Busy behaviour).
      ...
      if (!candidate) {
        toast.error(`No contra vouchers found on or before ${dateStr}`);
        return;
      }
      navigate(`/accounts/contra-entry/edit/${candidate.id}`, {
      ...
      <div className="w-full lg:w-[420px]">
        <div className="bg-card border border-line rounded-md overflow-hidden shadow-sm">
          <div className="bg-rose-600/90 text-white text-[13px] font-bold uppercase tracking-wide text-center py-1 border-b border-line">
            Select Voucher To Modify ( Contra )
          </div>
          <form id="modify-picker-form" onSubmit={handleSubmit} className="p-4 space-y-2.5 text-[13px]">
            ...
            <div className="grid grid-cols-12 gap-3 items-center">
              ...
              <div className="col-span-7">
                <input
                  ...
                  placeholder="e.g. 3"
                  className="w-full px-2 py-1 border border-line bg-slate-900 text-white rounded text-[13px] font-mono font-semibold focus:ring-1 focus:ring-rose-500/40 focus:border-rose-500 focus:outline-none"
                />
              </div>
            </div>
            ...
            <div className="pt-2 flex justify-center">
              <button
                type="submit"
                disabled={submitting}
                className="px-6 py-1 bg-rose-600 hover:bg-rose-700 text-white rounded font-semibold text-[13px] transition cursor-pointer disabled:opacity-50"
              >
...
export default ContraVoucherModifyPickerPage;
