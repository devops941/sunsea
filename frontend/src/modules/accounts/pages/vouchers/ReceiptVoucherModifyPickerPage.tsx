...
// Busy-style "Select Voucher To Modify (Receipt)" picker.
...
// cache, we pass it via router state so the edit page opens instantly.
const ReceiptVoucherModifyPickerPage: React.FC = () => {
  ...
  const { data: cachedVouchers } = useListCache<Voucher>({
    cacheKey: `accounts:receipt-vouchers:modify-picker`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (vNoRaw) {
        // Normalise "3" → "RCT-3" so operators can type the short display form.
        ...
        if (cacheHit) {
          navigate(`/accounts/receipt-voucher/edit/${cacheHit.id}`, {
        ...
        if (!found) {
          toast.error(`Voucher "${vNoRaw}" not found in Receipt`);
          return;
        }
        navigate(`/accounts/receipt-voucher/edit/${found.id}`, {
          state: { voucher: found },
        });
        return;
      }
      ...
      if (!candidate) {
        toast.error(`No receipt vouchers found on or before ${dateStr}`);
        return;
      }
      navigate(`/accounts/receipt-voucher/edit/${candidate.id}`, {
          ...
          <form id="modify-picker-form" onSubmit={handleSubmit} className="p-4 space-y-2.5 text-[13px]">
            ...
            <div className="grid grid-cols-12 gap-3 items-center">
              ...
              <div className="col-span-7">
                <input
                  ...
                  placeholder="e.g. 3"
                  className="w-full px-2 py-1 border border-line bg-card rounded text-[13px] text-ink focus:ring-1 focus:ring-emerald-500/40 focus:border-emerald-500 focus:outline-none"
                />
              </div>
            </div>
            ...
            <div className="pt-2 flex justify-center">
              <button
                type="submit"
                disabled={submitting}
                className="px-6 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded font-semibold text-[13px] transition cursor-pointer disabled:opacity-50"
              >
...
export default ReceiptVoucherModifyPickerPage;
