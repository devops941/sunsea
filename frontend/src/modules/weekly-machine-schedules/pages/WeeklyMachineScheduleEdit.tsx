...
const WeeklyMachineScheduleEdit: React.FC = () => {
    ...
    return (
        ...
        <CommonConfirmModal
            ...
            onConfirm={() => { setSaveConfirmOpen(false); if (proceedRef.current) { const p = proceedRef.current; proceedRef.current = null; resetRef.current = null; p(); return; } navigate(-1); }}
            title="Discard Changes?"
            message="You have unsaved changes. Are you sure you want to leave without saving?"
            confirmText="Discard"
            confirmVariant="danger"
            onCancel={() => { setSaveConfirmOpen(false); if (resetRef.current) { const r = resetRef.current; proceedRef.current = null; resetRef.current = null; r(); } setTimeout(() => lastFocusedRef.current?.focus(), 50); }}
        />
        </>
    );
};
