...
const WeeklyMachineScheduleEdit: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const loadProductionOrders = async () => {
            ...
            try {
                ...
                // Only "Ready for Planning" or equivalent status
                setProductionOrders(poList.filter((po: any) => 
        ...
        if (location.state) {
            if (location.state.machineId) {
                setSelectedMachineIds([location.state.machineId]);
            }
            if (location.state.weekStartDate) {
                const date = new Date(location.state.weekStartDate);
    ...
    const handleDragStart = (e: React.DragEvent, po: any) => {
    ...
    const handleDrop = async (e: React.DragEvent, dayValue: number, shiftId: string, targetMachineId: string) => {
        e.preventDefault();
        ...
        let currentAllPrograms = [
            ...weeklyPrograms.filter((wp: any) => 
                ...
                wp.status !== "CANCELLED" &&
                !deletedDbPrograms.includes(wp.weeklyProgramId)
            ),
        ...
        if (dragData.isGridMove) {
            ...
            // Find all blocks for this PO on this machine
            const existingBlocks = currentAllPrograms.filter((p: any) => p.productionOrderId === poId);

            // Reconstruct pseudo PO from existing blocks since we are moving them as a group
            const totalQtyToMove = existingBlocks.reduce((sum: number, item: any) => sum + Number(item.plannedQty || 0), 0);

            if (totalQtyToMove <= 0) {
                toast.error("Could not find any existing schedule blocks to move for this order.");
                return;
            }

            const po = {
                productionOrderId: poId,
                targetQty: totalQtyToMove,
                priority: existingBlocks[0]?.priority || "MEDIUM",
                productItem: existingBlocks[0]?.productionOrder?.productItem || { productName: "Unknown" }
            };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.error("Failed to save some schedules. Please check the logs.");
        } finally {
    ...
    const activeWeeklyPrograms = weeklyPrograms.filter((wp: any) => wp.weekStartDate.split("T")[0] === weekStartDate && wp.status !== "CANCELLED");

    const queryParams = new URLSearchParams(location.search);
    const filterPo = queryParams.get("productionOrderId") || queryParams.get("po");

    // Revert grouping completely, just filter if po is provided
    const displayOrders = filterPo 
    ...
    const getAllocatedQty = (productionOrderId: string) => {
        const dbAllocated = activeWeeklyPrograms
            .filter((wp: any) => wp.productionOrderId === productionOrderId)
            ...
            <Container fluid>
                <div className="page-header mb-4">
                    <Row className="align-items-center g-3">
                        <Col lg={6}>
                            <h2 className="page-title">Interactive Schedule Board</h2>
                            <div className="page-breadcrumb">Weekly Schedules / Edit</div>
                        </Col>
                        ...
                        <Col lg={12}>
                            <Card className="border-0 shadow-sm mb-3">
                                <Card.Header className="bg-white border-bottom">
                                    <h6 className="mb-0 fw-bold">Ready for Planning</h6>
                                        ...
                                        displayOrders.map((po: any) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="fw-bold text-primary mb-1">{po.productionOrderId}</div>
