...
const SupplierEdit: React.FC = () => {
    ...
    const handleClear = () => {
        if (supplierData) {
            setFormData({
                ...
                vendorType: supplierData.vendorType || "Manufacturer",
                category: supplierData.category ? supplierData.category.split(",").map((c: string) => c.trim()) : [],
                rawMaterialCategories: supplierData.rawMaterialCategories || "",
                ...
                gstRegType: supplierData.gstRegType || "Registered",
                msmeStatus: supplierData.msmeStatus || "None",
                udyamNo: supplierData.udyamNo || "",
                tdsSection: supplierData.tdsSection || "",
                billingAddressLine1: supplierData.billingAddressLine1 || "",
                billingAddressLine2: "",
                ...
                minOrderQty: Number(supplierData.minOrderQty) || 0,
                currency: supplierData.currency || "INR",
                bankAccounts: Array.isArray(supplierData.bankAccount)
                    ? supplierData.bankAccount.map((bank: any) => ({
                    ...
                    : supplierData.bankAccount && typeof supplierData.bankAccount === "object"
                        ? [
                            {
                                bankHolderName: supplierData.bankAccount.bankHolderName || "",
                                bankName: supplierData.bankAccount.bankName || "",
                                accountNumber: supplierData.bankAccount.accountNumber || "",
                                ifscCode: supplierData.bankAccount.ifscCode || "",
                                branchName: supplierData.bankAccount.branchName || "",
                                upiMobileNumber: supplierData.bankAccount.upiMobileNumber || "",
                                qrImage: supplierData.bankAccount.qrImage || "",
                            }
                        ]
                        : supplierData.bankAccount && typeof supplierData.bankAccount === "string"
                            ? [
                                {
                                    bankHolderName: supplierData.bankHolder || "",
                                    bankName: "",
                                    accountNumber: supplierData.bankAccount,
                                    ifscCode: supplierData.bankIfsc || "",
                                    branchName: "",
                                    upiMobileNumber: "",
                                    qrImage: "",
                                }
    ...
    useEffect(() => {
        if (supplierData && allRawMaterials.length > 0) {
            const selectedMaterialNames = supplierData.category
                ? supplierData.category.split(",").map((c: string) => c.trim())
                : [];

            // Find matching raw materials
            const matchedMaterials = allRawMaterials.filter((m: any) =>
                selectedMaterialNames.includes(m.materialName)
            );

            // Extract unique category IDs
            const uniqueCategoryIds = Array.from(new Set(
                matchedMaterials.map((m: any) => String(m.categoryId)).filter(Boolean)
            ));

            setSelectedParentCategories(uniqueCategoryIds);

            // Filter options for child dropdown
            const filteredMaterials = allRawMaterials.filter((m: any) =>
                uniqueCategoryIds.includes(String(m.categoryId))
            );
    ...
    }, [supplierData, allRawMaterials]);

    // Sync material prices when supplierData, allRawMaterials, and selected categories change
    useEffect(() => {
        if (supplierData && allRawMaterials.length > 0) {
            const dbPrices = (supplierData.materialPrices || []).filter((mp: any) => !mp.validTo);

            // Map db prices to state, looking up material name
            const mappedPrices = dbPrices.map((mp: any) => {
                const rm = allRawMaterials.find(m => String(m.rawMaterialId) === String(mp.rawMaterialId));
                return {
                    rawMaterialId: mp.rawMaterialId,
                    materialName: rm?.materialName || mp.materialName || `Material #${mp.rawMaterialId}`,
                    price: typeof mp.price === "string" ? parseFloat(mp.price) : (mp.price || 0),
                    validFrom: mp.validFrom ? mp.validFrom.split("T")[0] : new Date().toISOString().split("T")[0],
                    validTo: mp.validTo ? mp.validTo.split("T")[0] : "",
                };
            });

            setMaterialPrices((prev) => {
                // Keep only those that are currently selected in category (or newly added)
                // We also want to merge any modifications the user made locally on screen (in `prev`)
                const merged = mappedPrices.map((mp: any) => {
                    const localEdit = prev.find(p => p.rawMaterialId === mp.rawMaterialId);
                    return localEdit ? localEdit : mp;
                });

                const kept = merged.filter((p: any) => formData.category.includes(p.materialName));

                const existingNames = kept.map((p: any) => p.materialName);
                const newRows = formData.category
                    .filter((name: string) => !existingNames.includes(name))
                    .map((name: string) => {
                        const rm = allRawMaterials.find((m: any) => m.materialName === name);
                        return {
    ...
    }, [supplierData, allRawMaterials, formData.category]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await editSupplier(id, payload as any);
            ...
            if (Array.isArray(apiErrors) && apiErrors.length > 0) {
                ...
                const msg = typeof err === "string" ? err : err?.message || err?.response?.data?.message || "Failed to update supplier";
                toast.error(msg);
            }
        } finally {
            setIsSubmitting(false);
