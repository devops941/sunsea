...
interface PageShortcutOptions {
  ...
  onRefresh?: () => void;
  /** F6 — toggle alphabetical sorting on list pages. */
  onSort?: () => void;
 ...
 *
 * Usage:
 *   usePageShortcuts({ onRefresh: refetchData, onSort: toggleSort, onDelete: handleDeleteSelected });
 *   usePageShortcuts({ onNew: () => navigate('/create'), onExport: handleExport });
 */
export function usePageShortcuts({ onRefresh, onSort, onDelete, onNew, onExport }: PageShortcutOptions = {}) {
  ...
  useEffect(() => {
    ...
    const handleSort    = () => onSortRef.current?.();
    ...
    window.addEventListener(FKEY_EVENTS.SORT,    handleSort);
    ...
    return () => {
      ...
      window.removeEventListener(FKEY_EVENTS.SORT,    handleSort);
